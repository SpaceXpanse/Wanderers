...
char *strWndClass = "SpaceXpanse Render Window";
...
{
	hSpaceXpanseInst = g_pSpaceXpanse->GetInstance();
...
bool GraphicsClient::clbkInitialise ()
{
    ...
    WNDCLASS wndClass = {0, ::WndProc, 0, 0, hModule,
		LoadIcon (g_pSpaceXpanse->GetInstance(), MAKEINTRESOURCE(IDI_MAIN_ICON)),
		LoadCursor (NULL, IDC_ARROW),
		(HBRUSH)GetStockObject (WHITE_BRUSH),
		NULL, strWndClass};
    RegisterClass (&wndClass);

	if (clbkUseLaunchpadVideoTab() && g_pSpaceXpanse->Launchpad()) {
		hVid = g_pSpaceXpanse->Launchpad()->GetTabWindow(4);
		SetWindowLongPtr (hVid, GWLP_USERDATA, (LONG_PTR)this);
	} else hVid = NULL;

	// set default parameters from config data
	Config *cfg = g_pSpaceXpanse->Cfg();
	...
	((spacexpanse::DefVideoTab*)g_pSpaceXpanse->Launchpad()->GetTab(PG_VID))->OnGraphicsClientLoaded(this, fname);
#endif
...
bool GraphicsClient::TexturePath (const char *fname, char *path) const
{
	...
	strcpy (path, g_pSpaceXpanse->Cfg()->CfgDirPrm.HightexDir);
	strcat (path, fname);
	...
	// try tex directory
	strcpy (path, g_pSpaceXpanse->Cfg()->CfgDirPrm.TextureDir);
	strcat (path, fname);
	if ((fh = _findfirst (path, &fd)) != -1) {
		_findclose (fh);
		return true;
...
{
	g_pSpaceXpanse->Cfg()->PTexPath(path, planetname);
	return true;
}
...
{
	return g_pSpaceXpanse->Cfg()->GetParam (paramtype);
}
...
#include "splash_cs.dat" // generated by checksplash <path>/SpaceXpanse2016.jpg splash_cs.dat
...
LRESULT GraphicsClient::RenderWndProc (HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	default:
		return g_pSpaceXpanse->MsgProc (hWnd, uMsg, wParam, lParam);
	}
...
{
#ifndef INLINEGRAPHICS
	return g_pSpaceXpanse->AttachGraphicsClient (gc);
#else
	MessageBox(NULL, "Warning: You are trying to load an external graphics client into SpaceXpanse.\r\nExternal graphics clients can only be used with the SpaceXpanse Server application (spacexpanse_ng.exe).\r\n\r\nPlease deactivate the graphics module in the Modules tab.",
		"SpaceXpanse: Graphics module activated", MB_ICONWARNING|MB_OK);
	LOGOUT_ERR("Graphics module activated in spacexpanse.exe");
	return false;
...
{
#ifndef INLINEGRAPHICS
	return g_pSpaceXpanse->RemoveGraphicsClient (gc);
#else
	return false;
#endif // !INLINEGRAPHICS
}